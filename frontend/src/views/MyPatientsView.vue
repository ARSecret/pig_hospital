...
<template>
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-4">
        <div class="col" v-for="patient in patients">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title text-nowrap">{{ patient.full_name }}</h5>
                    <p class="card-text text-nowrap">{{ patient.birthdate }}</p>
                    <RouterLink :to="{ name: 'case-records', params: { patientId: patient.id } }"
                        class="btn btn-primary d-flex">История болезни</RouterLink>
                </div>
            </div>
        </div>
    </div>
